function [hE,hV]=wgPlot_hilmi_us_temp(adjMat,coord,varargin)
...
plotParm={'markerSize',20,'lineWidth',0.1,'marker','.','MarkerEdgeColor',[1,0,0]};
siz=size(adjMat);
vrtxSiz=100;
edgeMap=cool;
vrtxMap=summer;
...
if eWeighted
  ...
  yStep=(maxY-minY)/20;
  %legY=minY-yStep;% Hilmi modified below
  legY=minY-yStep-0.1;
  ...
  for kk=1:neColor
    ...
    % Draw legend bar
    line([legLim(kk),legLim(kk+1)],[legY,legY]-3,'lineWidth',15,'color',edgeMap(kk,:));
  end  % for kk
  
  % Draw Legend Label
  text([legLim(1),legLim(end)]-1,[legY-0.5*yStep,legY-0.5*yStep]-3.15, ...
    {num2str(minEWt),num2str(maxEWt)},'HorizontalAlignment','center','FontSize',16,'FontName','Times New Roman');
else
    nSegment=length(ii);
    x=[coord(ii,1),coord(jj,1),nan(nSegment,1)]';
    y=[coord(ii,2),coord(jj,2),nan(nSegment,1)]';
    hE=plot(x(:),y(:),'color',edgeMap(end,:),plotParm{:});  % hilmi modified
...
if vWeighted
  ...
  if exist('vrtxCol','var')
    ...
    hV=scatter(coord(:,1),coord(:,2),vWt,'filled','MarkerFaceColor',[1,0.5,0.2]);
  end
