function [hE,hV]=wgPlot_grid(adjMat,coord,varargin)
...
qq=unique([ii,jj]);
% minEWt=min(eWt);
minEWt=floor(min(eWt)*2)/2;  % Hilmi modified
maxEWt=max(eWt);
...
if eWeighted
  ...
  for kk=1:neColor
    ...
    PP = plot(x(:),y(:),'color',edgeMap(kk,:),plotParm{:});
    if ~isempty(PP) 
      hE=[hE,PP];
    end
    % Draw legend bar
  ...
  text([legLim(1),legLim(end)],[legY-0.5*yStep,legY-0.5*yStep], ...
    ...
    hE=plot(x(:),y(:),'color',edgeMap(end,:),plotParm{:});
end  % if eWeighted
